...
\begin{table}
	\begin{center}
		\begin{tabular}{l|c|r}
			Estación meteorológica & $A_{0}$ &  Valor medio\\
			\hline
...
Del total de las ciudades muestradas, se realiza un primer filtro para considerar aquellas que cumplan que su velocidad media sea mayor  a  $\frac{5m}{s}$, ya que la velocidad mínima para poder utilizar el viento como fuente de energia ronda los 3 o 4  $\frac{m}{s}$ dependiendo del aerogenerador. Luego, aplicamos un segundo filtro en el cual se consideran las ciudades que posean oscilaciones de menor amplitud, es decir, que $ \delta =  \sqrt{ A_1^2 + B_1^2 } $ sea mínimo, y que además $ \mu \pm \frac{\delta}{2} \in [4,25]$ 
