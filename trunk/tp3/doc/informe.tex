...
En la figura \ref{lst:finiteDifferences}, se detalla la implementación de un programa que, dados $\Delta r$ y $\Delta t$, utiliza las condiciones de contorno y la inicial de las ecuaciones \eqref{equation:vcontour1}, \eqref{equation:vcontour2} y \eqref{equation:vinitial} para armar parte de la matriz $v$ que representa a la función $v_{k}^{m}$, donde $k$ es la columna y $m$ es la fila. Para completar el resto, usa el esquema de la ecuación \eqref{equation:diffapprox}. \\
