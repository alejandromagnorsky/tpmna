...
Utilizando el vector de frecuencias generado a partir de la función de la figura \ref{lst:getFrequencies}, la función de la figura \ref{lst:writePartiture} genera un vector de caracteres que representan los tripletes de cada una de las frecuencias.\\

Cabe destacar que debido a la naturaleza aproximativa del proceso de obtención de las frecuencias de un archivo de sonido, es necesario que tanto los métodos \textit{frequencyToNote} (figura \ref{lst:frequencyToNote}) como \textit{noteFromDistance} (figura \ref{lst:noteFromDistance}), utilizados directa e indirectamente por el algoritmo \ref{lst:writePartiture}, trabajen con cierto grado de error al momento de decidir el intervalo de frecuencias (octava de una nota) al cual pertenece una frecuencia dada, así como la distancia en medio tonos de cada nota respecto a un \textit{la} en la octava cuarta -\textit{A-4} en notación de tripletes-, respectivamente.\\
...
compuesto por $n$ tripletes, la función \textit{readPartiture} de la figura \ref{lst:readPartiture} se encarga de transformar cada uno de ellos en la nota que les corresponda.
Una vez finalizado dicho procedimiento (\textit{noteToFrequency}, figura \ref{lst:noteToFrequency}), se utiliza cada frecuencia $f_k$ para 
construir un vector de valores $\mathbf{v^{f_k}}$, en donde el componente $v^{f_k}_i$ de dicho vector corresponde al resultado de la siguiente ecuación:
\begin{equation}
 v^{f_k}_i = \sin(2\pi t_i f_k)
\end{equation}
siendo $t_i$ el $i$-ésimo componente de un vector tiempo $t$ cuyos elementos son cada instante en un intervalo entre $0$ y $30 ms$ con un
paso $\tfrac{1}{f_s}$ entre instantes dado.

Como resultado, se disponen de $n$ vectores $\mathbf{v^{f_k}}$ que son finalmente concatenados en uno único, listo para
ser convertido a sonido mediante la función $wavwrite$ que ofrece Octave.
